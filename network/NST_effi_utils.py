...
def calculate_output_image_size(input_image_size, stride):
    ...
    if input_image_size is None:
        return None
    image_width = get_width_and_height_from_size(input_image_size)
    stride = stride if isinstance(stride, int) else stride[0]
    image_width = int(math.ceil(image_width / stride))
    return image_width  ## check : list or int
...
class Conv1dDynamicSamePadding(nn.Conv1d):
    ...
    def forward(self, x):
        iw = x.size()[-1:]
        kw = self.weight.size()[-1:][0]
        sw = self.stride
        ow = math.ceil(iw / sw)  # change the output size according to stride ! ! !
...
class Conv1dStaticSamePadding(nn.Conv1d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, image_size=None, **kwargs):
        ...
        self.stride = self.stride[0] if (
                    isinstance(self.stride, tuple) or isinstance(self.stride, list)) else self.stride
        # self.dilation = self.dilation[0] if isinstance(self.dilation, tuple) else self.dilation

        # Calculate padding based on image size and save it
        assert image_size is not None
        iw = image_size[0] if isinstance(image_size, tuple) else image_size
        ...
        pad_w = max((ow - 1) * self.stride + (kw - 1) * self.dilation[0] + 1 - iw, 0)
        if pad_w > 0:
            self.static_padding = ZeroPad1d((pad_w // 2, pad_w - pad_w // 2))
        else:
...
def get_same_padding_maxPool1d(image_size=None):
    """Chooses static padding if you have specified an image size, and dynamic padding otherwise.
       Static padding is necessary for ONNX exporting of models.
    ...
    """
    if image_size is None:
        return MaxPool1dDynamicSamePadding
    else:
        return partial(MaxPool1dStaticSamePadding, image_size=image_size)


class MaxPool1dDynamicSamePadding(nn.MaxPool1d):
    ...
    def __init__(self, kernel_size, stride, padding=0, dilation=1, return_indices=False, ceil_mode=False):
        ...
        # self.self.dilation[0] if isinstance(self.dilation, tuple) else self.dilation
...
class MaxPool1dStaticSamePadding(nn.MaxPool1d):
    ...
    def __init__(self, kernel_size, stride, image_size=None, **kwargs):
        ...
        # Calculate padding based on image size and save it
        assert image_size is not None
        iw = image_size  # , image_size) if isinstance(image_size, int) else image_size
        kw = self.kernel_size
        sw = self.stride
        ow = math.ceil(iw / sw)
        pad_w = max((ow - 1) * self.stride + (kw - 1) * self.dilation[0] + 1 - iw, 0)
...
def efficientnet(width_coefficient=None, depth_coefficient=None, image_size=None,
                 dropout_rate=0.35, drop_connect_rate=0.35, num_classes=2):
    ...
    blocks_args = [
        ...
        'r2_k5_s22_e6_i12_o24_se0.25',
        'r3_k3_s22_e6_i24_o80_se0.25',
        'r3_k5_s11_e6_i80_o112_se0.25',
        'r2_k5_s22_e6_i112_o160_se0.25',
        'r2_k5_s22_e6_i160_o192_se0.25',
