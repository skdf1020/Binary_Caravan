...
from .NST_effi_utils import (  ## .utils-> utils
    round_filters,
    round_repeats,
    drop_connect,
...
class MBConvBlock(nn.Module):
    ...
    def forward(self, inputs, drop_connect_rate=None):
        ...
        if self.has_se:
            x_squeezed = F.adaptive_avg_pool1d(x, 1)  ## check
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, gradcam=False):
        super().__init__()
        ...
        # Stem
        # in_channels = 1  # FHR
        in_channels = 2  # FHR + UC
        out_channels = 16  # round_filters(16, self._global_params)  # first layer before MBConv layer
        ...
        out_channels = round_filters(320, self._global_params)  ## check : 맨마지막 filter
        Conv1d = get_same_padding_conv1d(image_size=image_size)
        ...
        self.structure = list(map(str, blocks_args))
    ...
    def extract_endpoints(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            if prev_x.size(2) > x.size(2):  ##check
    ...
    def from_pretrained(cls, model_name, weights_path=None, advprop=False,
        ...
        load_pretrained_weights(model, model_name, weights_path=weights_path, load_fc=(num_classes == 2),
                                advprop=advprop)
        model._change_in_channels(in_channels)
        return model
...
#         """
#         if in_channels != 3:
#             Conv1d = get_same_padding_conv1d(image_size = self._global_params.image_size)
